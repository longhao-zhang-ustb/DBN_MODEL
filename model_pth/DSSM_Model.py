...
class DSSMClassifier(nn.Module):
    def __init__(self, input_dim, output_dim, dropout):
        ...
        #####################GRU#############################
        # encoder_layer = TransformerEncoderLayer(self.emb_dim + 512, self.nhead, dim_feedforward=(self.emb_dim + 512) * 4)
        # self.transformer = TransformerEncoder(encoder_layer, num_layers=2)
        self.att_layer = SelfAttentionLayer(self.emb_dim + 512, n_heads=self.nhead, dropout=dropout, device="cpu")
        self.att_linear2 = nn.Linear(self.emb_dim + 512, self.d_model)
        #####################XLSTM############################
    ...
    def forward(self, x):
        ...
        ch2_x = torch.cat((ch1_x, ch2_x), dim=1)
        ch2_x = self.dropout(ch2_x)
        # ch2_x = self.transformer(ch2_x)
        ch2_x = self.att_layer(ch2_x, ch2_x, ch2_x)
        ch2_x = self.att_linear2(ch2_x)
        ch2_x = self.dropout(ch2_x)
        ch2_x = ch2_x.view(ch2_x.shape[0], 1, -1)
